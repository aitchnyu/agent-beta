...
- ``/apps/<app>/e/<function>`` — dispatched by the request method to the
...
- ``/apps/<app>`` — resolves the function named ``default`` (the app's home).
  Only GET serves it; a non-GET on the app root is a 404 (the same
  404-everywhere invariant as a method/name mismatch on the named route). PATCH
  and other verbs the router doesn't list never reach the handler — ninja
  answers those with its own 405.
...
``DynamicModule`` call API. Unknown app/function → 404.
"""
...
# app_default_endpoint), so a name under the wrong method — or a non-GET app
# root — is a 404, not ninja's 405. There is no annotated Pydantic body param in
# either signature, so ninja leaves request.body / request.POST for the handler
# to read.
_ENDPOINT_PATH = "/{app_name}/e/{function_name}"
_DEFAULT_PATH = "/{app_name}"
...
def app_endpoint(
    ...
    return _serve(request, Application.app_or_404(app_name), function_name)
...
def app_default_endpoint(
    ...
    """Serve the app root as the function named ``default`` (GET only).

    Only GET serves ``default``; a non-GET app root is a 404 to honour the
    ...
    if request.method != "GET":
        raise Http404
    return _serve(request, Application.app_or_404(app_name), "default")
