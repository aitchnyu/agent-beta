...
``parent/main`` + ``parent/copy`` temp repos and patch
``djangoapp.views.git_data._REPO_ROOT`` to ``main`` — mirroring the real layout
(``main/`` = the dev-server repo, ``copy/`` = the scratch sibling
...
uncommitted changes (a ``-``/``+`` modification + an untracked file). Copy: a
...
def _worktree_roots(test: TestCase) -> tuple[Path, Path]:
    """Create a temp ``parent/main`` + ``parent/copy``; patch ``_REPO_ROOT``→main.

    Returns ``(main_root, copy_root)`` — both empty (no identity, no commits);
    callers :func:`_init_repo` each and build history. The temp dir is removed
    and the patch undone on test exit (registered via ``test.addCleanup``).
    """
    tmp = tempfile.TemporaryDirectory()
    test.addCleanup(tmp.cleanup)
    main_root = Path(tmp.name) / "main"
    copy_root = Path(tmp.name) / "copy"
    patcher = patch("djangoapp.views.git_data._REPO_ROOT", main_root)
    patcher.start()
    test.addCleanup(patcher.stop)
    return main_root, copy_root
...
class GitRepoMixin(TestCase):
    """Cooperative mixin: builds the shared main+copy repo fixture + ``_REPO_ROOT`` patch.
    ...
    - Copy uncommitted: TodoApp/copy_only.py modified, TodoApp/copy_notes.md untracked
    """
    ...
    def setUp(self) -> None:
        super().setUp()
        main_root, copy_root = _worktree_roots(self)
        ...
        # copy worktree — baseline commit + uncommitted changes.
        copy = _init_repo(copy_root, email="copy@example.com", name="Copy")
        _commit(copy, copy_root, {"TodoApp/copy_only.py": "x = 0\n"}, "scratch baseline")
        _write(copy_root, "TodoApp/copy_only.py", "x = 0\nx = 1\n")
        _write(copy_root, "TodoApp/copy_notes.md", "wip\n")
