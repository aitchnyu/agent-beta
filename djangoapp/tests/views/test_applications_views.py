...
class ApplicationsViewsTests(
    ...
    assert the served component and that props mirror the DB — apps listed and
    /manage row counts matching live rows. Non-superusers get 404 (never 403).

    - test_apps_page_superuser, AppList component + props mirror the DB apps
    - test_apps_page_non_superuser_404, non-superuser gets 404
    ...
    def test_apps_page_superuser(self) -> None:
        """AppList component renders and props mirror the DB apps."""
        self.client.force_login(self.superuser)
        self.client.get("/manage/apps")
        self.assertComponentUsed("AppList")
        props = self.props()["props"]
        self.assertEqual([a["name"] for a in props["apps"]], [_APP])

    def test_apps_page_non_superuser_404(self) -> None:
        """Non-superuser gets 404 (must not learn the page exists)."""
        self.client.force_login(self.plain)
        resp = self.client.get("/manage/apps")
    ...
    def test_manage_page_shows_row_counts(self) -> None:
        ...
        self.client.force_login(self.superuser)
        self.client.get(f"/manage/apps/{_APP}")
