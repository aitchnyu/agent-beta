...
from djangoapp.models.base import User, UserHistory, UserSnapshot
...
class UserModelTests(TestCase):
    ...
    def test_public_id_auto_generated_on_create_user(self) -> None:
        """create_user sets a non-empty public_id on creation."""
    ...
    def test_has_public_profile_defaults_false(self) -> None:
        """A freshly created user has has_public_profile set to False."""
        user = User.objects.create_user(username="alpha", password="pass")
    ...
    def test_search_users_ranks_by_best_match(self) -> None:
        ...
        # "alice" (exact) ranks above "alicia"
...
class UserHistoryModelTests(TestCase):
    ...
    def test_update_records_edited_history(self) -> None:
        ...
        self.assertEqual(
            entry._changes["first_name"],  # JSON diff field
            {"old": "Old", "new": "New"},
        )
    ...
    def test_difference_only_includes_changed_fields(self) -> None:
        """The edited diff marks unchanged fields as null (mirrors ArticleHistoryContent)."""
        self.user.update(
        ...
        changes = UserHistory.objects.get(target_user=self.user)._changes  # JSON diff field
    ...
    def test_record_created_snapshots_all_fields(self) -> None:
        ...
        self.assertEqual(
            entry._changes["has_public_profile"],  # JSON diff field
            {"old": False, "new": False},
        )
        self.assertEqual(
            entry._changes["email"],  # JSON diff field
            {"old": "old@example.com", "new": "old@example.com"},
        )
